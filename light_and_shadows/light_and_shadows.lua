...
function light_and_shadows.render_target(name, w, h, no_depth)
    ...
    local rt = render.render_target(name,
        {
            [graphics.BUFFER_TYPE_COLOR0_BIT] = color_params,
            [graphics.BUFFER_TYPE_DEPTH_BIT] = not no_depth and
                depth_params or nil
        })
...
function light_and_shadows.set_orthographic_projection(proj_w, proj_h, near_z, far_z)
    light_projection = vmath.matrix4_orthographic(-proj_w / 2, proj_w / 2, -proj_h / 2, proj_h / 2, near_z or -500,
        far_z or 500)
    is_ortho_proj = true
end
...
function light_and_shadows.render_shadows(self)
    ...
    render.disable_material()
    render.enable_material("shadow_instanced_billboard",
        { frustum = frustum, frustum_planes = render.FRUSTUM_PLANES_ALL })
