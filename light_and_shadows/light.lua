...
function light.update(self, dt)
    ...
    for id, obj in pairs(light.list) do
        ...
        if not obj.static then
            local wp = go.get_world_position(id) - obj.offset

            -- Simple candle light sway logic.
            -- Moves on the X-axis half the set candle_sway amount
            -- to positive and negative direction.
            if obj.candle_sway > 0 then
                obj.offset_x_dir = obj.offset_x_dir or 1
                obj.offset_x = obj.offset_x or 0
                obj.offset_x = obj.offset_x + ((obj.candle_sway * dt * 2) * obj.offset_x_dir)

                if obj.offset_x >= obj.candle_sway * 0.5 then
                    obj.offset_x_dir = -1
                elseif obj.offset_x <= -(obj.candle_sway * 0.5) then
                    obj.offset_x_dir = 1
                end
            end

            obj.position.x = wp.x + (obj.offset_x or 0)
            obj.position.y = wp.y + (obj.y_offset or 0)
            obj.position.z = wp.z + (obj.offset_x or 0)
        ...
        table.insert(lights, n,
            {
                position = obj.position,
                color = obj.current_value,
