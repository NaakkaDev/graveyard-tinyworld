...
vec3 adjustBCSH(vec3 col, float brightness, float contrast, float hue,
                float saturation) {
  // Adjust brightness
  col = col + brightness;
...
void main() {
  vec2 uv = (var_texcoord0 - 0.5 * resolution.xy) / resolution.y;
  uv.y -= 0.5;
  uv.y /= 1.1;
  uv.x *= 1000.0;
  float t = time.x / 50.0;
  ...
  sub.y = 0.24 - sub.y;
  float rain = sub.y;
  float glow = smoothstep(0.22, 1.0, rain) * 50.0;
  rain = clamp(rain, 0, 1.0);

  float gradientFactor =
      fract(fuv.x * 0); // Adjust the multiplier for gradient frequency
  vec3 RrainColor = getColorFromNoise(fuv) * 0.2;
  vec3 rainColor = sampleGradient(gradientFactor);
  rainColor = (rainColor + RrainColor) / 2.0;
  ...
  if (bright > thresholdForGlow) {
    float glow = smoothstep(0.22, 1.0, rain) * 50.;
    col += glow * 6.0;
  }
  ...
  frag_color = mix(color, vec4(col, 0), 0.1);
}