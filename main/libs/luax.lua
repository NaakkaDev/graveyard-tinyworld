...
---Does not copy metatables → if your tables have custom behavior via metatables, this won’t preserve it.
---Does not deepcopy keys (if you use tables as keys).
---@param source_table any
---@param _cyclic_cache any
---@return table
function M.table.deep_copy(source_table, _cyclic_cache)
    ...
    for key, value in pairs(source_table) do
        if type(value) == "table" then
            ...
            result[key] = _cyclic_cache[value] or M.table.deep_copy(value, _cyclic_cache)
        else
            result[key] = value
        end
    end
...
---No cyclic reference handling → will cause infinite recursion if tables point back to themselves.
--- table.deepcopy
...
---@param key_to_ignore any Optionally ignore one key, e.g. quantity for inventory item
function M.table.equals(o1, o2, ignore_mt, key_to_ignore)
    ...
    for key1, value1 in pairs(o1) do
        if key1 ~= key_to_ignore then
            local value2 = o2[key1]
            if value2 == nil or M.table.equals(value1, value2, ignore_mt) == false then
                return false
            end
            keySet[key1] = true
        end
    ...
    for key2, _ in pairs(o2) do
        if (key2 ~= key_to_ignore) and not keySet[key2] then return false end
    end
    return true
end
